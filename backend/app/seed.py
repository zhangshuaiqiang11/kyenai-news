...
ARTICLE_DEPTH: dict[str, DepthProfile] = {
    ...
    "cursor-enterprise-organizations-governance": {
        ...
        "analysis": "The useful change is not only administrative scope. Cursor Enterprise security depends on connected controls: identity, role permissions, repository allowlists or blocklists, model access, MCP server controls, subprocessors, Privacy Mode, and retention exceptions. That makes the page a security decision guide rather than a short changelog rewrite.",
        "implications": "Enterprise admins should map teams, identity groups, cost centers, repositories, model policies, MCP servers, and agent permissions before turning on broader access. The strongest early use is separating experimental groups from production-facing teams while still verifying Privacy Mode, retention terms, and audit evidence.",
        "reader_decision": "This update is most relevant for buyers searching whether Cursor Enterprise is secure enough for company code. The decision is not yes or no from a headline; it is whether the buyer can verify admin controls, retention behavior, subprocessors, and permission boundaries for their own workspace.",
        "limits": "Limits and open questions: public pages do not establish every account-specific security default, retention exception, export field, or customer-agreement term. Those should be verified through admin docs, trust-portal evidence, and a controlled enterprise trial before stronger claims are made.",
    },
...
def seed_articles() -> list[Article]:
    return [
        ...
        article(
            ...
            "Cursor Enterprise Security: Governance, Privacy Mode, and Agent Permissions",
            "cursor-enterprise-organizations-governance",
            ...
            "Security & Governance",
            ["cursor", "enterprise", "security", "privacy-mode", "agent-permissions"],
