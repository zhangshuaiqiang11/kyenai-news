import type { SearchCluster } from "./types";
...
export type GrowthEventParameters = {
  ...
  action_id?: string;
  resource_id?: string;
  /** Canonical measurement cluster; never send free-form query or user content. */
  search_cluster?: SearchCluster;
};

const IDENTIFIER_PATTERN = /^[a-z0-9][a-z0-9_-]{0,79}$/;
const PATH_PATTERN = /^\/(?:[a-z0-9][a-z0-9/._-]{0,158})?$/;
const SEARCH_CLUSTERS = new Set<SearchCluster>([
  "agent-instructions",
  "agent-workflows",
  "mcp-security",
  "cursor-governance",
...
function isSafeEventParameters(parameters: GrowthEventParameters): boolean {
  ...
  if (parameters.search_cluster !== undefined && !SEARCH_CLUSTERS.has(parameters.search_cluster)) return false;
