...
export function buildLlmsTxt(): string {
  ...
  const lines = [
    ...
    `- Agent instructions cluster: ${buildCanonicalUrl("/agent-instructions")} (instruction-file compatibility, templates, and adoption evidence)`,
    `- MCP cluster: ${buildCanonicalUrl("/mcp")} (tool discovery, setup, security, and dated MCP updates)`,
    `- Governance cluster: ${buildCanonicalUrl("/governance")} (approvals, auditability, and enterprise operating controls)`,
    `- Research and resources: ${buildCanonicalUrl("/research")} (versioned datasets, runbooks, security assessments, citation files, and templates)`,
    ...
    "",
    "## Citation guidance",
    `- Start at ${buildCanonicalUrl("/research")} for version, license, verification date, and preferred citation details.`,
