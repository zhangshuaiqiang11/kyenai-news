...
function printHelp() {
  console.log(`Generate the KyenAI instruction-file adoption snapshot.

Usage:
  GITHUB_TOKEN=... node generate-github-instruction-adoption.mjs [options]

Options:
  --sample-size <1-100>    Files to collect per query (default: 100)
  --snapshot-date <date>   Snapshot date in YYYY-MM-DD format (default: today)
  --output-dir <path>      Write the JSON and CSV to this directory
  --help                   Show this help

Without --output-dir, the script refreshes the website's public and bundled data files.`);
}
...
async function main(args = process.argv.slice(2)) {
  ...
  const sampleSizeArg = Number(readOption(args, "--sample-size"));
  const snapshotDateArg = readOption(args, "--snapshot-date");
  const outputDirArg = readOption(args, "--output-dir");
  ...
  const publicDir = outputDirArg
    ? path.resolve(outputDirArg)
    : path.join(root, "frontend", "public", "resources", "data");
  const dataDir = outputDirArg ? null : path.join(root, "frontend", "lib", "data");
  ...
  if (dataDir) fs.writeFileSync(path.join(dataDir, `${filename}.json`), json);
